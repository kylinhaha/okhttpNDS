...
public final class Cookie {
    ...
    /**
     * Attempt to parse a {@code Set-Cookie} HTTP header value {@code setCookie} as a cookie.
     * Returns
    ...
    static Cookie parse(long currentTimeMillis, HttpUrl url, String setCookie) {
        ...
        // If 'Max-Age' is present, it takes precedence over 'Expires', regardless of the order
        // the two
    ...
    private static long parseExpires(String s, int pos, int limit) {
        ...
        // If any partial is omitted or out of range, return -1. The date is impossible. Note
        // that leap
    ...
    /**
     * Returns the positive value if {@code attributeValue} is positive, or
     * {@link Long#MIN_VALUE} if
     ...
     * #persistent() not persistent}, in which case it will expire at the end of the current
     * session.
     * <p>
     ...
     * it should be interpreted as a pattern. This flag will be false if its {@code Set-Cookie}
     * header
     ...
     * example.com} and all subdomains including {@code api.example.com}, {@code www.example
     * .com}, and
     ...
     * this path's segments. For example, if this path is {@code /foo} this cookie matches
     * requests to
    ...
    /**
     * Returns true if this cookie should be included on a request to {@code url}. In addition to
     * this
