...
class OptionalMethod<T> {
    ...
    private static Method getPublicMethod(Class<?> clazz, String methodName, Class[]
            parameterTypes) {
        Method method = null;
        try {
    ...
    /**
     * Invokes the method on {@code target} with {@code args}. If the method does not exist or is
     * not
     * public then {@code null} is returned. See also
     * {@link #invokeOptionalWithoutCheckedException}.
     *
    ...
    public Object invoke(T target, Object... args) throws InvocationTargetException {
        Method m = getMethod(target.getClass());
        if (m == null) {
            throw new AssertionError("Method " + methodName + " not supported for object " +
                    target);
        }
        try {
            return m.invoke(target, args);
     ...
     * arguments must match those specified when the {@link OptionalMethod} was created. If the
     * return
