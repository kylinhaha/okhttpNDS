    ...
    public static Date parse(String value) {
        ...
        synchronized (BROWSER_COMPATIBLE_DATE_FORMAT_STRINGS) {
            for (int i = 0, count = BROWSER_COMPATIBLE_DATE_FORMAT_STRINGS.length; i < count; i++) {
                ...
                if (format == null) {
                    format = new SimpleDateFormat(BROWSER_COMPATIBLE_DATE_FORMAT_STRINGS[i],
                            Locale.US);
                    // Set the timezone to use when interpreting formats that don't have a
                    // timezone. GMT is
                ...
                if (position.getIndex() != 0) {
                    // Something was parsed. It's possible the entire string was not consumed but
                    // we ignore
                    // that. If any of the BROWSER_COMPATIBLE_DATE_FORMAT_STRINGS ended in
                    // "'GMT'" we'd have
