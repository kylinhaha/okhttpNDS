...
final class Hpack {
    ...
    static ByteString checkLowercase(ByteString name) throws IOException {
        for (int i = 0, length = name.size(); i < length; i++) {
            ...
            if (c >= 'A' && c <= 'Z') {
                throw new IOException("PROTOCOL_ERROR response malformed: mixed case name: " +
                        name.utf8());
            }
        }
        return name;
    ...
    static final class Reader {
        ...
        private int evictToRecoverBytes(int bytesToRecover) {
            ...
            if (bytesToRecover > 0) {
                ...
                for (int j = dynamicTable.length - 1; j >= nextHeaderIndex && bytesToRecover > 0;
                     j--) {
        ...
        /**
         * Read {@code byteCount} bytes of headers from the source stream. This implementation
         * does not
            ...
            while (!source.exhausted()) {
                ...
                if (b == 0x80) { // 10000000
                    ...
                    if (maxDynamicTableByteCount < 0
                        ...
                        throw new IOException("Invalid dynamic table size update " +
        ...
        private void insertIntoDynamicTable(int index, Header entry) {
            ...
            if (index == -1) { // Adding a value to the dynamic table.
                if (headerCount + 1 > dynamicTable.length) { // Need to grow the dynamic table.
                    ...
                    System.arraycopy(dynamicTable, 0, doubled, dynamicTable.length, dynamicTable
                            .length);
                    nextHeaderIndex = dynamicTable.length - 1;
    ...
    static final class Writer {
        ...
        /**
         * The decoder has ultimate control of the maximum size of the dynamic table but we can
         * choose
        ...
        int dynamicTableByteCount = 0;
        /**
         * In the scenario where the dynamic table size changes multiple times between
         * transmission of
        ...
        private int evictToRecoverBytes(int bytesToRecover) {
            ...
            if (bytesToRecover > 0) {
                ...
                System.arraycopy(dynamicTable, nextHeaderIndex + 1, dynamicTable,
                        nextHeaderIndex + 1 + entriesToEvict, headerCount);
                Arrays.fill(dynamicTable, nextHeaderIndex + 1, nextHeaderIndex + 1 +
                        entriesToEvict, null);
                nextHeaderIndex += entriesToEvict;
            }
            return entriesToEvict;
        ...
        void writeHeaders(List<Header> headerBlock) throws IOException {
            ...
            for (int i = 0, size = headerBlock.size(); i < size; i++) {
                ...
                if (headerIndex == -1) {
                    for (int j = nextHeaderIndex + 1, length = dynamicTable.length; j < length;
                         j++) {
