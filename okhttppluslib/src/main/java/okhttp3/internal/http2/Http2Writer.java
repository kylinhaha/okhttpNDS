...
final class Http2Writer implements Closeable {
     ...
     * sent as a part of the response to {@code streamId}. The {@code promisedStreamId} has a
     * priority
    ...
    public synchronized void synStream(boolean outFinished, int streamId,
                                       int associatedStreamId, List<Header> headerBlock) throws
            IOException {
        if (closed) throw new IOException("closed");
    ...
    /**
     * Tell the peer to stop creating streams and that we last processed {@code
     * lastGoodStreamId}, or
     ...
     */
    public synchronized void windowUpdate(int streamId, long windowSizeIncrement) throws
            IOException {
        if (closed) throw new IOException("closed");
        if (windowSizeIncrement == 0 || windowSizeIncrement > 0x7fffffffL) {
            throw illegalArgument("windowSizeIncrement == 0 || windowSizeIncrement > 0x7fffffffL:" +
                            " %s",
                    windowSizeIncrement);
        }
        int length = 4;
