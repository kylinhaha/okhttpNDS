...
public final class DiskLruCache implements Closeable, Flushable {
    ...
    public static DiskLruCache create(FileSystem fileSystem, File directory, int appVersion,
        ...
        Executor executor = new ThreadPoolExecutor(0, 1, 60L, TimeUnit.SECONDS,
                new LinkedBlockingQueue<Runnable>(), Util.threadFactory("OkHttp DiskLruCache",
                true));
    ...
    /**
     * Creates a new journal that omits redundant information. This replaces the current journal
     * if it
     * exists.
     */
    ...
    synchronized Editor edit(String key, long expectedSequenceNumber) throws IOException {
        ...
        if (mostRecentTrimFailed || mostRecentRebuildFailed) {
            ...
            // data than
            // requested by the user. Do not allow edits so we do not go over that limit any
            // further. If
            // the journal rebuild failed, the journal writer will not be active, meaning we will
            // not be
            // able to record the edit, causing file leaks. In both cases, we want to retry the
            // clean up
    ...
    /**
     * Returns the number of bytes currently being used to store the values in this cache. This
     * may be
        ...
        return new Iterator<Snapshot>() {
            ...
            public void remove() {
                ...
                try {
                    ...
                    // that's
                    // because we couldn't update the journal, but the cached entry will still be
                    // gone.
                } finally {
                    removeSnapshot = null;
                }
    ...
    public final class Editor {
         ...
         * causes
         * an I/O error, or if the target entry is evicted while this editor is active. In either
         * case
         * we delete the editor's created files and prevent new files from being created. Note
         * that once
        ...
        /**
         * Returns an unbuffered input stream to read the last committed value, or null if no
         * value has
         * been committed.
         */
