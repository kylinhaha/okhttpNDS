...
public final class Util {
     ...
     * decimal digits and dots. This pattern matches strings like "a:.23" and "54" that are
     * neither IP
     ...
     */
    public static boolean skipAll(Source source, int duration, TimeUnit timeUnit) throws
            IOException {
        long now = System.nanoTime();
        long originalDuration = source.timeout().hasDeadline()
                ? source.timeout().deadlineNanoTime() - now
                : Long.MAX_VALUE;
        source.timeout().deadlineNanoTime(now + Math.min(originalDuration, timeUnit.toNanos
                (duration)));
        try {
    ...
    private static boolean containsInvalidHostnameAsciiCodes(String hostnameAscii) {
        for (int i = 0; i < hostnameAscii.length(); i++) {
            ...
            // definition for OkHttp's host header checks (and the WHATWG Host syntax definition)
            // . Here
