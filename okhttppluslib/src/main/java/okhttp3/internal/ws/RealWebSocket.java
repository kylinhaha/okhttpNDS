...
public final class RealWebSocket implements WebSocket, WebSocketReader.FrameCallback {
     ...
     * read and all outgoing messages have been written. It is closed when both reader and writer
     * are
    ...
    /**
     * When executed this will cancel this websocket. This future itself should be canceled if
     * that is
    ...
    public void connect(OkHttpClient client) {
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) {
                ...
                Streams streams = streamAllocation.connection().newWebSocketStreams
                        (streamAllocation);
    ...
    synchronized boolean close(int code, String reason, long cancelAfterCloseMillis) {
        ...
        if (reason != null) {
            ...
            if (reasonBytes.size() > CLOSE_MESSAGE_MAX) {
                throw new IllegalArgumentException("reason.size() > " + CLOSE_MESSAGE_MAX + ": "
                        + reason);
            }
        }
    ...
    /**
     * Attempts to remove a single frame from a queue and send it. This prefers to write urgent
     * pongs
     * before less urgent messages and close frames. For example it's possible that a caller will
     * enqueue messages followed by pongs, but this sends pongs followed by messages. Pongs are
     * always
     * written in the order they were enqueued.
     * <p>
     * <p>If a frame cannot be sent - because there are none enqueued or because the web socket
     * is not
